  ...
  <Document>
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.section}>
        ...
        <Text style={styles.detail}>Date: {formatDate(ticketReceipt.date)}</Text>
        <Text style={styles.detail}>Time: {ticketReceipt.time}</Text>
        <Text style={styles.detail}>Room: {ticketReceipt.room}</Text>
        <Text style={styles.detail}>Seats: {ticketReceipt.seats.join(", ")}</Text>
      </View>
        ...
        <View style={styles.section}>
          ...
          {ticketReceipt.menuItems.map((item, index) => (
            <View key={index} style={styles.row}>
              <Text>{item.quantity} x {item.name || `Item ${index + 1}`}</Text>
              <Text style={styles.bold}>Rs. {(item.price * item.quantity).toFixed(2)}</Text>
            </View>
          ))}
        </View>
      )}
      ...
      <View style={styles.section}>
        ...
        <Text style={styles.detail}>Reference ID: {ticketReceipt.referenceId}</Text>
        <Text style={styles.detail}>Booking ID: #{ticketReceipt.bookingId}</Text>
        <Text style={styles.detail}>Total Amount: Rs. {ticketReceipt.totalPrice.toFixed(2)}</Text>
      </View>
      ...
      {ticketReceipt.loyaltyPointsEarned > 0 && (
        <View style={styles.section}>
          <Text style={[styles.row, { justifyContent: 'space-between' }]}>
            ...
            <Text style={styles.bold}>+{ticketReceipt.loyaltyPointsEarned}</Text>
          </Text>
        </View>
      )}
  ...
  useEffect(() => {
    const completeBooking = async () => {
      try {
        ...
        const newBookingId = bookingResponse.data.bookingId || bookingResponse.data._id || bookingResponse.data.data?._id || bookingResponse.data.booking?._id || "Unknown";
        ...
        try {
          ...
          const roomResponse = await axios.post("http://localhost:3001/api/room/therater", { room_id: pendingBooking.room_id });
          ...
          if (roomResponse.data && roomResponse.data.success && roomResponse.data.room) {
            ...
            if (roomDetails.showtimes && roomDetails.showtimes.length > 0) {
              ...
              const matchedShowtime = roomDetails.showtimes.find((st) => st._id === pendingBooking.time_slot);
        ...
        if (!showtimeDetails) {
          try {
            console.log(`Trying to fetch showtime directly with ID: ${pendingBooking.time_slot}`);
            const showtimeResponse = await axios.get(`http://localhost:3001/api/showtime/${pendingBooking.time_slot}`);
            ...
            try {
              console.log(`Trying room/showtimes endpoint for room: ${pendingBooking.room_id}`);
              const roomShowtimesResponse = await axios.get(`http://localhost:3001/api/room/${pendingBooking.room_id}/showtimes`);

              if (roomShowtimesResponse.data && roomShowtimesResponse.data.showtimes) {
                const showtimes = roomShowtimesResponse.data.showtimes;
                const matchedShowtime = showtimes.find((st: Showtime) => st._id === pendingBooking.time_slot);

                if (matchedShowtime) {
                  showtimeDetails = matchedShowtime;
                  setShowtime(matchedShowtime);
                  console.log("Found showtime in room/showtimes API:", showtimeDetails);
                }
              }
        ...
        try {
          console.log(`Fetching movie details for movie ID: ${pendingBooking.movie_id}`);
          const movieResponse = await axios.get(`http://localhost:3001/api/movie/${pendingBooking.movie_id}`);

          if (movieResponse.data && (movieResponse.data.movie || movieResponse.data.success)) {
        ...
        try {
          if (pendingBooking.menu_items && pendingBooking.menu_items.length > 0) {
            ...
            menuItemsDetails = pendingBooking.menu_items.map((item: any) => {
              const menuItemDetails = allMenuItems.find((menuItem: any) => menuItem._id === item.menu_id) || {};
        ...
        try {
          ...
          if (loyaltyPointsEarned > 0) {
            try {
              const checkPointsResponse = await axios.get(`http://localhost:3001/api/loyalty-points/user/${pendingBooking.user_id}`);

              if (checkPointsResponse.status === 200) {
                await axios.put(`http://localhost:3001/api/loyalty-points/user/${pendingBooking.user_id}`, {
                  points: checkPointsResponse.data.data.points + loyaltyPointsEarned,
                });
                console.log(`Added ${loyaltyPointsEarned} loyalty points to user account`);
              }
              ...
              if (pointsError.response?.status === 404) {
                try {
                  ...
                  console.log(`Created loyalty points account with ${loyaltyPointsEarned} points`);
                } catch (createPointsError) {
                  console.error("Error creating loyalty points:", createPointsError);
                }
              } else {
        ...
        const finalRoomName = roomName || pendingBooking.roomName || "Theater Room";

        const receipt: TicketReceipt = {
          bookingId: newBookingId,
          movieTitle: movieDetails?.title || pendingBooking.movie_title || "Movie",
          date: formattedDate,
          time: timeDisplay,
          room: finalRoomName,
          seats: pendingBooking.seats,
          ...
          menuItems: menuItemsDetails.length > 0 ? menuItemsDetails : pendingBooking.menu_items,
    ...
    return (
      <div className="container mx-auto p-4 sm:p-8 text-center">
        <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-md">
          ...
          <button className="mt-4 px-4 py-2 bg-yellow-500 text-white rounded-md hover:bg-yellow-600" onClick={() => navigate("/")}>
            Return to Home
          </button>
        </div>
      </div>
    ...
    return (
      <div className="container mx-auto p-4 sm:p-8 md:py-12 print-hide">
        <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded-md mb-8 print-hide">
          <svg className="h-12 w-12 sm:h-16 sm:w-16 text-green-500 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
          </svg>
          <h2 className="text-xl sm:text-2xl font-bold text-green-700 mb-2">Payment Successful!</h2>
          ...
          {pointsEarned > 0 && (
            <div className="mt-4 flex items-center justify-center gap-x-2 text-primary">
              ...
              <p className="font-medium">You earned {pointsEarned} loyalty points!</p>
            </div>
          )}
        </div>

        <div className="bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden max-w-3xl mx-auto" style={{ borderTop: "8px solid #FBC700" }} id="ticket-receipt">
          <div className="p-4 sm:p-6 text-center border-b">
            ...
            <p className="text-gray-500 text-sm sm:text-base">Your order has been successfully placed</p>
          </div>
          ...
          <div className="p-4 sm:p-6">
            <div className="mb-6">
              <div className="flex items-start">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xl font-bold">{ticketReceipt.movieTitle}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mb-6">
              <div className="flex items-start">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-600">{formatDate(ticketReceipt.date)}</p>
                  <p className="text-gray-800 font-medium">{ticketReceipt.time}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mb-6">
              <div className="flex items-start">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-800 font-medium">{ticketReceipt.room}</p>
                </div>
              </div>
            </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap mt-2">
                    {ticketReceipt.seats.map((seat) => (
                      <span key={seat} className="inline-block px-3 py-1 mr-2 mb-2 rounded-full text-sm font-medium" style={{ backgroundColor: "#fff5d7", color: "#e3b400" }}>
                        {seat}
                      </span>
                    ))}
                  </div>
                        ...
                        <div key={index} className="bg-gray-50 rounded-lg p-3 flex justify-between items-center">
                          <div className="flex items-center">
                            <div className="h-8 w-8 rounded-full bg-white flex items-center justify-center mr-3 shadow-sm">
                              <span className="font-medium" style={{ color: "#e3b400" }}>{item.quantity}</span>
                            </div>
            ...
            <div className="mb-6">
              <div className="flex items-start">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-600">Payment Method: <span className="font-medium">eSewa</span></p>
                  <p className="text-gray-600">Reference ID: <span className="font-medium">{ticketReceipt.referenceId}</span></p>
                  <p className="text-gray-600">Booking ID: <span className="font-medium">#{ticketReceipt.bookingId}</span></p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 rounded-xl p-4" style={{ backgroundColor: "#fff5d7" }}>
              <div className="flex justify-between items-center mb-2 text-base">
                ...
                <span className="font-bold" style={{ color: "#e3b400" }}>Rs. {ticketReceipt.totalPrice.toFixed(2)}</span>
              </div>
              ...
              {ticketReceipt.loyaltyPointsEarned > 0 && (
                <div className="flex justify-between items-center text-sm pt-2 border-t border-yellow-200 mt-2">
                  ...
                  <span className="font-medium text-green-600">+{ticketReceipt.loyaltyPointsEarned} points</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="px-4 pb-6 sm:px-6">
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <PDFDownloadLink document={<TicketPDF ticketReceipt={ticketReceipt} />} fileName="ticket_receipt.pdf">
              ...
              <button onClick={handleGoHome} className="flex items-center justify-center gap-2 px-4 py-2 rounded-lg font-medium text-white transition duration-300 transform hover:scale-105" style={{ backgroundColor: "#FBC700", boxShadow: "0 4px 12px rgba(251, 199, 0, 0.3)" }}>
                Continue to Home
              </button>
            </div>
          </div>
